...
async function main() {
  ...
  // --- Loại ca (Bước 1: 明番/早番/中番/遅番/超早/超遅/午前番) ---
  const shiftTypes = [
    ...
    // 新規: 月次シフト調整に追加する2つの補助シフト（不足時は1人）
    { code: "午前番", name: "Ca buổi sáng", defaultStartTime: "08:00", defaultEndTime: "14:00", spansMidnight: false },
  ];
  ...
  // v8（2026-09）: INC→責任者(RESPONSIBLE)→STAFF→CONTRACT→PARTTIME→OJT の順に変更。
  // 「責任者」ロールを新規追加。自動アサインではINC・責任者はOFFICEを優先的に担当し、
  // 業務A/B/全の必要人数を満たせない場合にのみ他スタッフと同様に業務A/B/全へ回る
  // （詳細は src/lib/autoAssign.ts・docs/AUTO_ASSIGN_RULES.md 参照）。
  const rolePriorities: { role: "INC" | "RESPONSIBLE" | "STAFF" | "CONTRACT" | "PARTTIME" | "OJT"; priorityOrder: number }[] = [
    { role: "INC", priorityOrder: 1 },
    { role: "RESPONSIBLE", priorityOrder: 2 },
    { role: "STAFF", priorityOrder: 3 },
    { role: "CONTRACT", priorityOrder: 4 },
    { role: "PARTTIME", priorityOrder: 5 },
    { role: "OJT", priorityOrder: 6 },
  ];
