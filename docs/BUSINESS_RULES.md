...
## 休憩ルール（通常勤務、2026-09 v8で全面改訂）

- **6時間以下の勤務には休憩を配置しない。**
- **7時間以上勤務した場合は休憩1時間を配置する**（勤務開始から3〜5時間程度を目安に配置）。
- 6時間超7時間未満のシフトは運用上存在しない前提。
- **パートタイムも対象**（v8で変更。以前はパートタイムを休憩対象外としていたが撤廃した。
  パートタイムが7時間以上勤務する場合は他の従業員と同じく休憩1時間を付与する）。
- スタッフ全員を同時に休憩に入れない（時間をずらす）のを優先するが、それによって業務A/B/全の
...
- **最初から事務時間を配置しない。** 業務A/B/全・WHILL関連業務・休憩をすべて配置したあとに
  余った時間だけを使う。
- パートスタッフは事務時間に配置禁止。
- **v8で追加**: INC・責任者(`EmployeeRole.RESPONSIBLE`)は通常時は事務時間を優先的に担当する。
  ただしINC・責任者を事務時間専任に固定するわけではなく、業務A/B/全の必要人数を現有スタッフ
  だけで満たせない場合は、INC・責任者も他の従業員と同様に業務A/B/全を担当する。
...
- 休憩: v8で変更。7時間以上勤務する場合は他の従業員と同じく休憩1時間を付与する
  （以前はパートタイムは休憩対象外だったが撤廃）。

## INC・責任者（`EmployeeRole.INC` / `EmployeeRole.RESPONSIBLE`、2026-09 v8）

- `責任者`（`RESPONSIBLE`）はv8で新規追加したロール。INCは元々プロジェクトに存在するロール。
- ロール優先順位のデフォルト（`RolePriority`初期値）: **INC → 責任者 → STAFF → CONTRACT →
  PARTTIME → OJT**。
- 通常時、INC・責任者は事務時間(OFFICE)を優先的に担当する。
- ただしINC・責任者は事務時間専任ではない。業務A/B/全の必要人数を現有スタッフだけでは
  満たせない場合、INC・責任者も他の従業員と同様に業務A/B/全を担当する（最終手段としての
  「INC投入」は、この既存roster内でのINC・責任者の業務A/B/全参加によって実現しており、
  ロースターへの新規追加・呼び出しは自動アサインの範囲外）。
- 人員不足の解消は、まず現在のroster内で調整することを優先し、無理な細かいローテーションや
  無理な配置による解決はしない。
...
- `EmployeeRole`（従業員としての役割）: `STAFF`（社員）/ `CONTRACT`（契約社員）/
  `PARTTIME`（バイト）/ `OJT` / `INC` / `RESPONSIBLE`（責任者。2026-09 v8で追加）。
  INCは実際に自動アサインの対象として運用されている（「現状未使用」ではない）。
