...
- `EmployeeRole`（業務上の役割。UserRoleとは別軸）: `STAFF` / `CONTRACT` / `PARTTIME` / `OJT` /
  `INC` / `RESPONSIBLE`（責任者。2026-09 v8で追加）。自動アサインのパート制限
  （WHILL・事務時間禁止）、およびINC・責任者のOFFICE優先ルールはこちらの `EmployeeRole` を見る
  （`src/lib/autoAssign.ts`、docs/AUTO_ASSIGN_RULES.md参照）。
- `src/middleware.ts` が保護対象ページへの未ログインアクセスをブロックする。

## 外部連携

- **メール**: `nodemailer` 経由（`src/lib/mailer.ts`）。月間スケジュール確定時の通知等。
- **AI機能**: `openai` を使った `POST /api/roster/ai-assist`（月間ローテーションのAI提案）と
