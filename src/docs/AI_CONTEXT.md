...
- **重要**: 2026-08-07に追加したPrisma schema変更（`ShiftClaimRequest.desiredStartTime/EndTime`,
  `CartPosition.slotUnitMinutes`, `EmployeePriority`）を使う前に、
  `npx prisma generate && npx prisma db push`（または`npx prisma migrate dev`）の実行が必要。
  開発時のsandbox環境がネットワーク制限によりPrismaの生成コマンドを実行できなかったため、
  未実行のままコードだけが存在している状態。
