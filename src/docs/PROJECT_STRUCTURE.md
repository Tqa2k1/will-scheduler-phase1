...
│   │   ├── shift-requests/      page.tsx  — 勤務希望申請 + 希望勤務(KIBO)登録（ShiftClaimRequest）
│   │   ├── shift-adjustment/    page.tsx  — シフト調整（管理者専用。2026-08追加）
│   │   ├── tasks/               page.tsx  — 業務（CartPosition）・業務要件・優先順位管理
...
│   │   ├── dailyStaffing.ts     # 1日の必要人数 vs 出勤人数の集計（日単位の粗い指標）
│   │   ├── shiftWindowStaffing.ts # 1日12人＝早番/遅番/明番 各4人のダイヤ単位充足（2026-08追加）
│   │   ├── kiboWindow.ts        # 希望勤務(KIBO)の受付期間判定（2026-08追加）
...
│   ├── middleware.ts          # 認証必須ページの保護（/dashboard /employees /roster /schedule /tasks /users /shift-requests /shift-adjustment）
