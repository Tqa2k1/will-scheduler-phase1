...
const ASSIGNABLE_ROLES = ["INC", "STAFF", "CONTRACT", "PARTTIME", "OJT"];
...
export default function TasksPage() {
  ...
  async function handleUpdateColor(task: Task, color: string) {
    await fetch(`/api/cart-positions/${task.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleUpdateSlotUnit(task: Task, slotUnitMinutes: 30 | 60) {
    await fetch(`/api/cart-positions/${task.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ slotUnitMinutes }),
    });
    load();
  }
              ...
              >
                ...
                <td style={{ padding: "8px 12px" }} onClick={(e) => e.stopPropagation()}>
                  <input
                    ...
                    onChange={(e) => handleUpdateColor(t, e.target.value)}
                ...
                <td style={{ padding: "8px 12px" }}>
                  {t.category === "CART" ? "カート業務" : "特殊区分"}
                </td>

                <td style={{ padding: "8px 12px" }}>
                  {t.operatingStartTime && t.operatingEndTime
                    ? `${t.operatingStartTime}〜${t.operatingEndTime}`
                    : "—"}
                </td>
                ...
                <td style={{ padding: "8px 12px" }} onClick={(e) => e.stopPropagation()}>
                  <select
                    className="input"
                    value={t.slotUnitMinutes ?? 60}
                    onChange={(e) => handleUpdateSlotUnit(t, Number(e.target.value) as 30 | 60)}
                    style={{ padding: "3px 6px", fontSize: 12, width: 90 }}
                    title="日別スケジュールの手動編集で、この業務を30分単位でも配置できるようにするか"
                  >
                    <option value={60}>1時間（基本）</option>
                    <option value={30}>30分も可</option>
                  </select>
                </td>

                <td style={{ padding: "8px 12px" }}>
                  {t.requirements.length}件
                </td>
...
function CreateTaskModal({
  ...
  const [slotUnitMinutes, setSlotUnitMinutes] = useState<30 | 60>(60);
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="label">時間単位</label>
        <select
          className="input"
          value={slotUnitMinutes}
          onChange={(e) => setSlotUnitMinutes(Number(e.target.value) as 30 | 60)}
          style={{ marginBottom: 10 }}
        >
          <option value={60}>1時間（基本。ほとんどの業務はこちら）</option>
          <option value={30}>30分も可（日別スケジュールで30分単位のコマとして配置できる）</option>
        </select>

        <label className="label">業務の色</label>
        <div
          style={{
            display: "flex",
            alignItems: "center",
...
function PriorityPanel({
  ...
  return (
    <div className="card">
      ...
      >
        自動アサイン時にどの役割を優先して割り当てるかの順序です（INCを含む全役割が対象です）。
      </p>
...
// ---------------------------------------------------------------------------
// 従業員個人の優先順位（「役割優先順位」を補完する機能。2026-08追加）
// ---------------------------------------------------------------------------
// 上の PriorityPanel（役割単位）とは別物。雇用区分（正社員/契約社員/バイト等）ごとに
// グループ分けし、各グループの中で従業員個人の優先順位を設定する。
// 用途: ある時間帯で人員が不足した際、「同じ役割の中でどの従業員を優先的に補充要員として
// 選ぶか」だけに使う（担当業務A/B/全/WHILL等の決定には使わない。詳細は
// docs/BUSINESS_RULES.md「従業員個人優先順位」を参照）。
type EmployeePriorityItem = { employeeId: string; fullName: string; role: string; priorityOrder: number };

const PRIORITY_GROUP_ORDER = ["STAFF", "CONTRACT", "PARTTIME", "OJT", "INC"];

function EmployeePriorityPanel() {
  const [items, setItems] = useState<EmployeePriorityItem[]>([]);
  const [loading, setLoading] = useState(false);

  async function load() {
    const res = await fetch("/api/employee-priorities");
  ...
  const grouped = PRIORITY_GROUP_ORDER.map((role) => ({
    role,
    members: items.filter((i) => i.role === role).sort((a, b) => a.priorityOrder - b.priorityOrder),
  })).filter((g) => g.members.length > 0);

  function move(role: string, index: number, dir: -1 | 1) {
    const members = items.filter((i) => i.role === role).sort((a, b) => a.priorityOrder - b.priorityOrder);
    const target = index + dir;
    if (target < 0 || target >= members.length) return;
    [members[index], members[target]] = [members[target], members[index]];
    // このグループ内の並び順だけを更新し、他グループのpriorityOrderはそのまま保持する
    const reordered = members.map((m, i) => ({ ...m, priorityOrder: i }));
    const others = items.filter((i) => i.role !== role);
    setItems([...others, ...reordered]);
  }

  async function handleSave() {
    setLoading(true);
    // グループごとに連番を振り直してから保存する（グループをまたいだ比較はしない仕様のため、
    // 値そのものはグループ内で相対的に小さいほど優先、という意味だけを持てば良い）
    const payloadItems: { employeeId: string; priorityOrder: number }[] = [];
    for (const role of PRIORITY_GROUP_ORDER) {
      const members = items.filter((i) => i.role === role).sort((a, b) => a.priorityOrder - b.priorityOrder);
      members.forEach((m, idx) => payloadItems.push({ employeeId: m.employeeId, priorityOrder: idx + 1 }));
    }
    await fetch("/api/employee-priorities", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      <h2 style={{ fontFamily: "var(--font-display)", fontSize: 16, marginTop: 0 }}>
        役割優先順位 — 従業員個人の優先順位
      </h2>
      <p style={{ color: "var(--color-text-muted)", fontSize: 13, marginTop: -6 }}>
        雇用区分（正社員/契約社員/バイト等）ごとに、その中での個人の優先順位を設定します。
        これは「ある時間帯の人員が不足したとき、同じ役割の中で誰を優先的に補充要員として選ぶか」
        だけに使われます（担当業務A/B/全/WHILL等の決定には使いません）。
      </p>

      <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
        {grouped.map((g) => (
          <div key={g.role} style={{ minWidth: 220 }}>
            <h3 style={{ fontSize: 14, marginBottom: 6 }}>{ROLE_LABEL[g.role] ?? g.role}</h3>
            <ol style={{ paddingLeft: 0, listStyle: "none" }}>
              {g.members.map((m, idx) => (
                ...
                >
                  <span style={{ width: 18, color: "var(--color-text-muted)", fontSize: 12 }}>{idx + 1}</span>
                  <span style={{ flex: 1, fontSize: 13 }}>{m.fullName}</span>
                  <button
                    className="btn-secondary"
                    onClick={() => move(g.role, idx, -1)}
                  ...
                  <button
                    ...
                    onClick={() => move(g.role, idx, 1)}
                    style={{ padding: "1px 7px", fontSize: 11 }}
                    disabled={idx === g.members.length - 1}
                  >
                    ↓
                  </button>
                </li>
