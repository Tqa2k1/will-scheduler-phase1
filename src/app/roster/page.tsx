...
type ShortageBand = { shiftTypeId: string; shiftTypeCode: string; shiftLabel: string; shortage: number };
type ShortageInfo = { confirmed: boolean; confirmedAt?: string; shortagesByDate: Record<string, { totalShortage: number; bands: ShortageBand[] }> };
type MyKiboRequest = { workDate: string; status: "PENDING" | "APPROVED" | "REJECTED" };
...
export default function RosterPage() {
  ...
  const [shortageInfo, setShortageInfo] = useState<ShortageInfo>({ confirmed: false, shortagesByDate: {} });
  const [myKiboRequests, setMyKiboRequests] = useState<MyKiboRequest[]>([]);
  const [confirmingShortage, setConfirmingShortage] = useState(false);
  const [kiboModalDate, setKiboModalDate] = useState<string | null>(null);
  ...
  async function load() {
    const [rosterRes, shiftRes, patternRes, shortageRes] = await Promise.all([
      fetch(`/api/roster?month=${monthStr}`),
      fetch(`/api/shift-types`),
      fetch(`/api/rotation-patterns`),
      fetch(`/api/roster/shift-adjustment/confirm?month=${monthStr}`),
    ]);
    if (rosterRes.ok) {
    ...
    if (shortageRes.ok) setShortageInfo(await shortageRes.json());

    if (!isAdmin) {
      const claimsRes = await fetch(`/api/shift-claims`);
      if (claimsRes.ok) {
        const data = await claimsRes.json();
        setMyKiboRequests(data.myRequests ?? []);
      }
    }
  }
  ...
  async function handleConfirmShortage() {
    if (!confirm(`${monthStr} の現在確定しているシフトを基に人員不足を再計算し、社員の月間勤務表に表示します。よろしいですか？`)) return;
    setConfirmingShortage(true);
    const res = await fetch("/api/roster/shift-adjustment/confirm", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ month: monthStr }),
    });
    ...
    if (res.ok) {
      const data = await res.json();
      alert(`人員不足を確定しました（不足あり: ${data.dateCount}日 / 合計不足人数: ${data.totalShortage}人）`);
      load();
    } else {
      alert("人員不足の確定に失敗しました。");
    }
  }
    ...
    <div style={{ maxWidth: 1500, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "8px 0 20px", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {isAdmin && (
            <button className="btn-secondary" onClick={handleConfirmShortage} disabled={confirmingShortage}>
              {confirmingShortage ? "確定中..." : "人員不足を確定"}
            </button>
          )}
          {isAdmin && (
      ...
      {shortageInfo.confirmed && (
        <p style={{ color: "var(--color-text-muted)", fontSize: 12, marginTop: -12, marginBottom: 12 }}>
          人員不足の表示は {new Date(shortageInfo.confirmedAt!).toLocaleString("ja-JP")} 時点でINCが確定した内容です。
          {isAdmin && " シフトを変更した場合は「人員不足を確定」を再度押すと最新の内容に更新されます。"}
        </p>
      )}
      ...
      <div className="card" style={{ overflowX: "auto", padding: 0 }}>
        <table style={{ width: "max-content" }}>
          <thead>
            ...
            <tr>
              <th style={{ ...thStyle, position: "sticky", left: 0, zIndex: 30, minWidth: 120, width: 120, background: "var(--color-surface-2)", fontWeight: 400, fontSize: 11, color: "var(--color-text-muted)" }}>
                人員不足
              </th>
              {days.map((d) => {
                const dateStr = `${year}-${pad2(month)}-${pad2(d)}`;
                const dayShortage = shortageInfo.shortagesByDate[dateStr];
                const myStatus = myKiboRequests.find((r) => r.workDate.slice(0, 10) === dateStr)?.status;
                if (!dayShortage || dayShortage.totalShortage <= 0) {
                  return <th key={d} style={{ ...thStyle, minWidth: 56 }} />;
                }
                return (
                  <th key={d} style={{ ...thStyle, minWidth: 56 }}>
                    {isAdmin ? (
                      <span style={{ fontSize: 11, color: "var(--color-danger)", fontWeight: 600 }}>
                        {dayShortage.totalShortage}人不足
                      </span>
                    ) : myStatus ? (
                      <span style={{ fontSize: 11, color: myStatus === "APPROVED" ? "var(--color-ok)" : myStatus === "REJECTED" ? "var(--color-text-muted)" : "var(--color-accent)" }}>
                        {myStatus === "PENDING" ? "申請済み" : myStatus === "APPROVED" ? "承認済み" : "却下"}
                      </span>
                    ) : (
                      <button
                        onClick={() => setKiboModalDate(dateStr)}
                        style={{
                          fontSize: 11,
      ...
      {kiboModalDate && (
        <KiboApplyModal
          workDate={kiboModalDate}
          bands={shortageInfo.shortagesByDate[kiboModalDate]?.bands ?? []}
          shiftTypes={shiftTypes}
          onClose={() => setKiboModalDate(null)}
          onApplied={() => {
            setKiboModalDate(null);
            load();
          }}
        />
      )}
...
// 希望勤務(KIBO)申請モーダル。人員不足が確定している日について、社員がどの勤務帯に
// 対応可能かを選び、「何時から何時まで勤務できるか」を指定して申請する。
// 申請後はINCが承認/却下する（既存の承認フローと同じ ShiftClaimRequest を使う）。
function KiboApplyModal({
  workDate, bands, shiftTypes, onClose, onApplied,
}: {
  workDate: string;
  bands: ShortageBand[];
  ...
  const [selectedShiftTypeId, setSelectedShiftTypeId] = useState(bands[0]?.shiftTypeId ?? "");
  const selectedShiftType = shiftTypes.find((s) => s.id === selectedShiftTypeId);
  const [startTime, setStartTime] = useState(selectedShiftType?.defaultStartTime ?? "");
  const [endTime, setEndTime] = useState(selectedShiftType?.defaultEndTime ?? "");
  const [submitting, setSubmitting] = useState(false);

  function selectBand(band: ShortageBand) {
    setSelectedShiftTypeId(band.shiftTypeId);
    const st = shiftTypes.find((s) => s.id === band.shiftTypeId);
    setStartTime(st?.defaultStartTime ?? "");
    setEndTime(st?.defaultEndTime ?? "");
  }
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="card" style={{ width: 380, maxWidth: "90vw" }}>
        <h3 style={{ marginTop: 0, fontSize: 16 }}>希望勤務（KIBO）申請 — {workDate}</h3>
        <p style={{ color: "var(--color-text-muted)", fontSize: 12, marginTop: -6 }}>
          対応できる勤務帯を選び、実際に勤務できる時間を指定して申請してください。管理者が確認のうえ承認します。
        </p>

        <label className="label">不足している勤務帯</label>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
          {bands.map((b) => (
            <label key={b.shiftTypeId} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
              <input
                type="radio"
                name="kibo-band"
                checked={selectedShiftTypeId === b.shiftTypeId}
                onChange={() => selectBand(b)}
              />
              {b.shiftLabel}（{b.shortage}人不足）
            </label>
          ))}
        </div>

        <label className="label">実際に勤務できる時間</label>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button className="btn" onClick={submit} disabled={submitting || !selectedShiftTypeId || !startTime || !endTime}>
            {submitting ? "送信中..." : "申請する"}
          </button>
        </div>
      </div>
