...
type PendingClaim = {
  ...
  shiftType: { code: string; defaultStartTime: string; defaultEndTime: string } | null;
...
export default function ShiftRequestsPage() {
  ...
  async function handleDecision(id: string, decision: "APPROVED" | "REJECTED") {
    const res = await fetch(`/api/shift-claims/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px" }}>
      ...
      {isAdmin ? (
            ...
            <table style={{ width: "100%" }}>
              <thead>
                <tr style={{ textAlign: "left", color: "var(--color-text-muted)", fontSize: 13 }}>
                  ...
                  <th style={{ padding: "10px 12px" }}>希望時間</th>
                  <th style={{ padding: "10px 12px" }}>操作</th>
                </tr>
              </thead>
              <tbody>
                {pending.length === 0 && (
                  <tr><td colSpan={5} style={{ padding: 16, textAlign: "center", color: "var(--color-text-muted)" }}>承認待ちの申請はありません</td></tr>
                )}
                {pending.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      {p.requestedStartTime && p.requestedEndTime ? `${p.requestedStartTime}〜${p.requestedEndTime}` : "—"}
                    </td>
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: 24, background: "#eff6ff", borderColor: "#bfdbfe" }}>
            <h2 style={{ fontSize: 15, marginTop: 0 }}>希望勤務登録（KIBO）</h2>
            <p style={{ color: "var(--color-text-muted)", fontSize: 13, margin: 0 }}>
              希望勤務（KIBO）の登録は<Link href="/roster" style={{ fontWeight: 600 }}>月間勤務表</Link>から行えるようになりました。
              INCがシフト調整を完了すると、月間勤務表の該当日に不足人数・不足している勤務時間帯が表示され、
              そこから対応できる時間を指定して申請できます。
            </p>
          </div>
