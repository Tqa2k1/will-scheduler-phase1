...
export async function GET(req: NextRequest) {
  ...
  const pageWidth = nameColWidth + shiftColWidth + timeColWidth + slots.length * slotColWidth + 40;
  const rowHeight = 18;
  const pageHeight = 80 + (rosterItems.length + 1) * rowHeight + 40;
  ...
  for (const r of rosterItems) {
    x = 20;
    doc.text(r.employeeName, x, y, { width: nameColWidth });
    x += nameColWidth;
    doc.text(r.isCarryOver ? "明番(引継)" : r.shiftTypeCode ?? "", x, y, { width: shiftColWidth });
    x += shiftColWidth;
    doc.text(r.resolvedStart && r.resolvedEnd ? `${r.resolvedStart}-${r.resolvedEnd}` : "", x, y, { width: timeColWidth });
    x += timeColWidth;
    slots.forEach((s, i) => {
