...
const InputSchema = z.object({
  ...
  selections: z.array(
    z.object({
      ...
      date: z.string(),
      shiftLabel: z.string(),
      shiftTypeId: z.string().optional(), // 依頼を ShiftClaimRequest として記録するために使用
    })
  ),
});
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 依頼を送った時点で ShiftClaimRequest（返答待ち=PENDING）として記録する。
    // これにより、次回「シフト調整を実行」した際、同じ勤務（同じ日）に既に依頼済みの
    // 従業員（返答待ち/承認済み/却下済みのいずれか）は候補者一覧に再表示されない
    // （findCandidatesForShift 側で除外している）。
    // 既に記録がある場合（本来は候補者一覧から除外されているため通常は発生しない）は
    // 二重作成を避けるためスキップする。
    for (const s of selections) {
      try {
        await prisma.shiftClaimRequest.create({
          data: {
            employeeId: s.employeeId,
            workDate: new Date(s.date + "T00:00:00Z"),
        ...
        if (err?.code !== "P2002") {
          console.error(`[shift-adjustment/send-emails] 依頼記録の作成に失敗: ${s.employeeId} ${s.date}`, err);
        }
        // P2002（employeeId+workDateの一意制約違反）＝既に依頼済み。何もせずスキップする。
      }
    }
